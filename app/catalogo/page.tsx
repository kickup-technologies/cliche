...
// Familias olfativas: fuente única en lib/families (compartida con el editor
// de catálogo del panel admin).
...
function CatalogoInner() {
  ...
  useEffect(() => {
    fetch("/api/products")
      .then((r) => r.json())
      // Los aromas sacados de todas las familias desde el admin (categoría
      // "ninguna") no participan del catálogo ni de sus contadores.
      .then((data) => { setProducts(Array.isArray(data) ? data.filter((p) => familyOf(p) !== null) : []); setLoading(false) })
