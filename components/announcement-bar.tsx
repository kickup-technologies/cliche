...
export function AnnouncementBar() {
  ...
  return (
    <div className="bg-[#2D1A14] text-[#FAF8F5] relative">
      {/* px-10 reserva espacio para la X (derecha) y equilibra a la izquierda,
          evitando que el contenido (y el timer) se solape con el botón cerrar. */}
      <div className="container mx-auto flex items-center justify-center gap-2 sm:gap-3 px-10 py-2.5 text-[11px] sm:text-sm font-medium">
        <Zap className="w-3.5 h-3.5 text-[#C4958A] flex-shrink-0" />
        <span className="hidden sm:inline text-[#FAF8F5]/70 uppercase tracking-widest text-[10px] font-semibold flex-shrink-0">Oferta del día</span>
        <span className="hidden sm:block w-px h-3 bg-[#FAF8F5]/20 flex-shrink-0" />
        <span className="min-w-0 truncate font-semibold text-[#FAF8F5]" data-cliche-edit="announcement_text" data-cliche-label="Texto del anuncio">{announcement || <>Envío gratis en compras mayores a <span className="text-[#C4958A]">{freeShipLabel}</span></>}</span>
        <span className="hidden md:block w-px h-3 bg-[#FAF8F5]/20 flex-shrink-0" />
        <span className="hidden md:inline flex-shrink-0 text-[#FAF8F5]/80">Código <span className="font-bold text-[#C4958A] tracking-wider">{discountCode}</span> → {discountPct}% OFF</span>
        <span className="hidden md:block w-px h-3 bg-[#FAF8F5]/20 flex-shrink-0" />
